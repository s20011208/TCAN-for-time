...
class CSLA(nn.Module):
    ...
    def forward(self, x, real_len, attn_mask):
        ...
        state = (torch.zeros(self.num_layers * self.directional, x.size(0), self.hidden_dim, device=x.device),
                 torch.zeros(self.num_layers * self.directional, x.size(0), self.hidden_dim, device=x.device))
...
class TCAN(pl.LightningModule):
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        x, ts, adj, y = x.to(self.device), ts.to(self.device), adj.to(self.device), y.to(self.device)
        pre = self([x, real_len, ts, adj])
        loss = F.mse_loss(pre, y.view(-1, 1))
